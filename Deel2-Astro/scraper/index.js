...
async function scrapeSteamDeals() {
  ...
  const games = await page.$$eval(".search_result_row", (rows) => {
    return rows.map((row) => ({
      ...
      discountPercentage: row.querySelector(".discount_pct")
        ? parseInt(row.querySelector(".discount_pct").textContent.trim().replace("-", "").replace("%", ""))
        : null,
    }))
  })
  ...
  // Write ook een bestand naar frontend/src/data
